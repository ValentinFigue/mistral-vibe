...
- **[Experimental / internal]** Métier **`tutor` agent** — the student-facing half of the Teaching métier (`vibe --agent tutor`), a conversational tutor whose every turn is **hard-gated against the teacher-authored `TutorContract`**. This is the "join": the graph the teacher authored *is* the boundary the tutor provably runs inside. The tutor is deliberately **not** a graph author — it addresses the student only through a single `tutor_reply(move, draft)` tool (its sole egress), and each call is validated **in code** before the student sees it (`vibe/core/tools/builtins/_tutor.py`): the move must be in the contract's `allowed_moves` (so `answer` is structurally refused), a `hint` may not skip up the ladder, and a draft that would give away the solution outside the contract's reveal policy (`never` / `on_request` / `after_attempts` + attempt count) is **withheld from the student** — the model is told to give a smaller hint or escalate. Whether a draft reveals the answer is decided by a small **LLM-judge** over the draft (no answer key needed); it **fails closed** (no live session / ambiguous / REVEAL → block). A second tool `escalate_to_teacher` records a hand-off to `<session_dir>/tutor/escalations.jsonl` when the contract's escalation limit is reached. The contract's rules (objectives, reveal policy, ordered hint ladder, escalation triggers, done criteria, allowed/forbidden moves) are injected into the tutor's system prompt at session start (`_get_tutor_contract_section`, mirroring the graph catalog injection), loaded from the session's authored graph, a `*.json` path, or a cache fingerprint (cross-session). Enforcement uses only existing machinery (tool deny/rewrite + `ctx.sampling_callback` for the judge) — **no agent-loop changes**; `session_dir` is threaded into `get_universal_system_prompt` as an optional arg. Attempt counts + hint rung live in **session-scoped tool state**, never the shared cache (privacy). Scoped so only the `tutor` agent sees `tutor_reply`/`escalate_to_teacher`. Residual (v1): free assistant text outside the tool isn't gated (mitigated by prompt discipline; a core-loop output gate is noted as follow-up). No `CACHE_VERSION` bump.
- **[Experimental / internal]** Métier **`teacher` agent** — a third métier (`vibe --agent teacher`) that authors lessons on the *same* workflow-graph engine, further proving it is **artifact-agnostic**: a new artifact family (`Standard`, `ObjectiveSet`, `Passage`, `Item`/`ItemSet`, `Rubric`, and the boundary `TutorContract`) flows through the unchanged engine — handles, content-addressed cache, incremental recompute, and the `run_pipeline` DSL all work with no engine edits. The operator library (`vibe/core/graph/library/teaching.py`, tagged `library="teaching"`) is **domain-neutral** — the domain (subject, grade band, standard) lives in the *parameters*, not the operator names — shaped **frame → generate → adapt → assess → bound**: frame (`set_standard`, `sample_standard`, LLM `set_objectives`, `sequence_lesson`), generate (LLM `generate_passage`, `generate_items` — which takes a `Passage` *or* an `ObjectiveSet` via a union input; pure `build_worksheet`/`build_quiz`), adapt (LLM `adapt_reading_level`, `differentiate(level)`, `translate`, `add_scaffolds`), assess (LLM `build_rubric`, `map_to_objectives`; pure `make_answer_key`, `check_coverage`), and assemble (`assemble_unit`). The **bound** ops (`set_reveal_policy`, `define_hint_ladder`, `set_escalation_rules`, `set_done_criteria`, `export_contract`) chain into a first-class, content-addressed **`TutorContract`** — the boundary a future student-facing tutor would provably run inside (note the deliberate absence of an `answer` move: answer keys are for teachers, so `answer` is named in the contract's `forbidden_moves`). **Domain preset blocks** supply defaults over the neutral ops: `reading_lesson`, `quiz_from_standard`, `differentiated_worksheet`, and `tutor_contract` (a one-click contract). LLM ops route JSON replies through the same retry-on-empty helper as `doc-reviewer`; pure ops (renders, coverage checks, every bound op) run headless. Scoped catalog (the teacher sees only the `teaching` library; analyst/doc-reviewer/graph unchanged), a dedicated `teacher` system prompt, and bundled sample standards (`ccss_ela_5`, `ccss_math_6`, `ngss_ms_ls2`) so it runs with no external files. No `CACHE_VERSION` bump (a new, independent library).
- **[Experimental / internal]** Métier **`doc-reviewer` agent** — a second métier (`vibe --agent doc-reviewer`) that proves the workflow-graph engine is **artifact-agnostic**: instead of the analyst's tabular `Table`, a `Document` (any file split into `Section`s) flows through the *same* engine — handles, content-addressed cache, incremental recompute, and the `run_pipeline` DSL all work unchanged, with no engine edits (the handle keys off `type(result).__name__`, so a `Document` renders as `ref:<fp> Document` for free). The operator library (`vibe/core/graph/library/doc_reviewer.py`, tagged `library="documents"`) is **domain-neutral** — the domain (contracts, papers, policies, RFPs, …) lives in the *parameters*, not the operator names: load (`read_document` — `.txt`/`.md` native, `.pdf` via the lazy optional **`[pdf]`** extra; `sample_document`), clean/shape (`clean_document`, `chunk_document`, `filter_sections`, `search_sections`, `select_sections`, `expect_sections`), extract (LLM `extract_segments(categories)`, `extract_fields(fields)`), assess (LLM `classify(dimension, labels)` — one general classifier that stacks dimensions like risk/favorability/sentiment; `compare_to_reference(reference_path, criterion)`; the pure `find_missing`, `check_references`, `check_definitions`; grounded `answer_question`, `summarize_document`), and deliver (`outline`, `items_to_markdown`, `combine_reports`, LLM `suggest_edits`, `write_memo`). The artifact is a neutral `Item{section, category, text, labels, note}`. **Domain preset blocks** supply default params over the neutral ops: `inventory` (any doc), `contract_review` + `term_sheet` (legal), `paper_abstract` (research), `gap_check` (policy). LLM ops route JSON replies through a retry-on-empty helper (see the Fixed entry). Scoped catalog (the reviewer sees only the `documents` library; analyst/graph unchanged), a dedicated `doc_reviewer` system prompt, bundled sample documents, and a **comparison-harness** row (`python -m vibe.core.graph.demo.compare`) showing the same context / recompute wins on a `Document` workflow (~85% cold / ~99% re-run reduction). New optional `[pdf]` extra (`pypdf`, lazy-imported; `.txt`/`.md` need no extra). No `CACHE_VERSION` bump (a new, independent library).
