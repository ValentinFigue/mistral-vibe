...
- **[Experimental / internal]** Métier analyst — **DABench root-cause fixes: two correctness bugs, ML capability gaps, and a new multivariate-regression operator** (driven by a full 257-question InfiAgent-DABench run: 82.1% accuracy, weakest on Machine Learning 63.2% and Comprehensive Data Preprocessing 68.9%). **`_coerce_column` now recognizes standard NA tokens** (`NA`/`N/A`/`null`/`None`/`NaN`/…, case-insensitive, matching `pandas.read_csv`'s default `na_values`) instead of only the literal empty string — previously a single such token in an otherwise-numeric column silently downgraded the **whole column** to strings, and non-numeric NA tokens were invisible to every downstream `IS NULL`/`.isna()`/`group_by` (root cause of a real failing "mean grouped by null vs not-null" question). **A real user-visible behavior change**: a categorical column legitimately containing the value `"NA"` (e.g. a country/state code) now becomes null — deliberate, matching pandas' own well-known default. `filter_rows` gains **`is_null`/`is_not_null`** ops (previously there was no way to isolate null rows via `filter_rows` at all — `==`/`!=` against `""` either silently returned zero rows or raised on a numeric column). `ml_classification` gains **`class_weight="balanced"`** and **`solver`** (logreg/tree/rf/svc only, validated with a clear error otherwise) and fixes a **`max_iter` hardcoded to 1000** that contradicted the docstring's claim of reproducing sklearn defaults (real default: 100) — three concrete gaps that plausibly explained several "always slightly worse, never better" accuracy mismatches. One-hot encoding gains **`drop_first`** (threaded through `ml_regression`/`ml_classification`/`feature_importance`/`ml_cluster`/`ml_predict`) — not a no-op for L2-regularized models, and a common reference-solution convention. `_ml_xy`'s previously-silent `dropna()` now surfaces a **`notes`** field on the returned `Table` (e.g. "dropped 178/891 row(s) with a missing value in [...]"), rendered into the per-step schema line the agent already reads — additive, doesn't affect `answer()`'s 1×1 check. New operator **`regression_summary(table, target, features, intercept=True)`** — multivariate OLS via `numpy.linalg.lstsq` + `scipy.stats` (no new runtime dependency), returning `coef`/`std_err`/`t_stat`/`p_value` per feature; closes a real capability gap where a question needing a coefficient **controlling for** a confounder had no correct tool (the agent substituted a bivariate `corr_test`, sign-flipping the result — a textbook Simpson's-paradox failure mode, reproduced and pinned by a reference-parity test against `statsmodels.OLS`, added as a dev-only test dependency). Analyst-prompt additions: preserve a requested format string's exact delimiter/spacing verbatim in the final reply (recurring scoring false-negative — `a,b` vs required `a, b` — confirmed to be model-authored text, not any operator's output); prefer one `run_pipeline` submission chaining every step over many small ones (turns are finite, and hard/multi-step questions disproportionately failed); pass explicit params for a stated rule/threshold rather than relying on a default (`outliers` defaults to `method="iqr"`, not `"zscore"`); a `sql`-based null-vs-not-null grouping pattern; and explicit escalation guidance when no operator/param exists rather than substituting a similar-looking computation. **`CACHE_VERSION` bumped 4 → 5** (`_coerce_column`/`filter_rows`/`ml_classification` semantics changed) — invalidates every existing user's shared `~/.vibe/graph-cache-v4.sqlite`. `benchmarks/dabench/run_dabench.py` now classifies and persists a failure reason (`turn_limit_exceeded`/`price_limit_exceeded`/`timeout`/`nonzero_exit`) instead of collapsing every failure to an identical empty response; new `benchmarks/dabench/analyze_results.py` reproduces the categorized empty/correct/partial/wrong-by-level/concept breakdown as reusable tooling instead of one-off scripting.
- **[Experimental / internal]** Métier analyst — **InfiAgent-DABench evaluation harness** (`benchmarks/dabench/`). Fetches the 257-question closed-form DA-Agent benchmark (`setup.sh`, gitignored `vendor/`) and drives it via `run_dabench.py`, one `vibe` subprocess per question in an isolated temp workdir, scored by InfiAgent's own `eval_closed_form.py`. Adds `.vibe/agents/analyst-headless.toml` — a copy of the builtin `analyst` profile with `bypass_tool_permissions = true`, needed because headless `run_pipeline`/`graph_save_block` calls are otherwise silently skipped ("not permitted") with no approval callback registered; benchmark-only, not for interactive use. Per-question prompts append the question's `format` field so the free-text final reply contains the `@name[value]` placeholders DABench's scorer expects — no changes to the analyst's system prompt or answer surface. See `benchmarks/dabench/README.md`.
- **[Experimental / internal]** Métier analyst — **catalog grouped by category + operator consolidation** (selection quality & prompt de-duplication, not raw size). The `run_pipeline` catalog now renders **grouped under `[category]` headers** (`load`/`shape`/`transform`/`statistics`/`inference`/`ml`/`quality`/`sql`/`report`/`insight`) instead of a flat 57-op alphabetical list — a `category` field added to `OperatorSpec` + the `@operator` decorator, grouped by `render.operators_catalog`; **backward-compatible** (a catalog with no categories renders flat, so other libraries are unchanged). **Consolidated near-duplicates (57 → 50 ops):** `outliers_iqr`/`outliers_zscore` → **`outliers(method=iqr|zscore)`**, and the seven chart ops (`bar_chart`/`line_chart`/`scatter`/`histogram`/`box`/`pie`/`heatmap`) → **`chart(kind=…)`** (per-kind param validation; preserves line `series`, box `by`, pie `max_slices`, heatmap matrix). The analyst prompt's Compute section is trimmed from a full op-enumeration to **methodology** — the categorized catalog is now the single source of op names/params, ending the prose duplication (~1k tokens). **Breaking renames** for the experimental métier (no built-in blocks reference these ops); **no `CACHE_VERSION` bump** (renamed ops mint new recipes; old cache entries orphan harmlessly).
- **[Experimental / internal]** Métier analyst — **generic statistics & preprocessing operators** (DA-bench-informed, deliberately not overfit). Adds **inferential statistics** via `scipy.stats` (already present as a scikit-learn base dep, previously unused): `corr_test` (coefficient + **p-value** + n; pearson/spearman/kendall), `normality_test` (shapiro/normaltest/anderson), `group_test` (t-test/Welch/Mann-Whitney/ANOVA/Kruskal — does a metric differ across groups), `chi_square` (categorical independence). Adds **preprocessing transforms**: `normalize` (min-max / z-score), `encode` (label / one-hot as a real column transform), a z-score option on `outliers`, and a `mode` method on `fill_missing`. `correlation` gains a `method` param (pearson/spearman/kendall). The ML ops (`ml_regression`/`ml_classification`/`ml_cluster`/`feature_importance`/`ml_predict`) gain **`encode=onehot|label`** so the agent can match a question's stated feature encoding (one-hot vs label changes a linear model's numbers). Inference ops return small 1-row tables (report both coefficient + p_value via `to_markdown`, or slice one into `answer`). Deliberately generic — **no** benchmark answer-format helpers and **no** significance-verdict rubric (the agent composes "significant if p<α", "linear if |r|≥0.5 and p<0.05" itself from `r`+`p`). Faithfulness pinned by **scipy reference-parity tests** (each op == a direct scipy computation). No `CACHE_VERSION` bump (new ops; `encode` defaults to current one-hot behaviour, so existing ML recipes are unaffected).
- **[Experimental / internal]** Métier analyst — **DA-bench Machine-Learning push**. The `ml_*` operators now reproduce scikit-learn faithfully enough to answer benchmark ML questions ("fit model X, report the MSE/RMSE"). **scikit-learn is now a base dependency** (dropped the optional `[ml]` extra — no flag, `ml_*` always works; PyInstaller specs bundle it unconditionally, CI no longer needs `--extra ml`). `ml_regression` gains models `lasso`/`gbr`/`knn`/`svr` and metrics `mse`/`mape` (now `r2|rmse|mse|mae|mape`); `ml_classification` gains `gbm`/`knn`/`svc`/`nb` and metrics `precision`/`recall` (weighted, `zero_division=0`). Both gain **`evaluate` ∈ `holdout`|`full`|`cv`** (held-out split · fit+score on all rows for no-split questions · k-fold mean) and a **`scale`** flag (StandardScaler, leak-free). New **`ml_predict`** returns per-row predictions (`[*features, target, {target}_pred]`), index-aligned. The `seed` param is renamed **`random_state`** (default `42`, the sklearn convention; kept a concrete int so ops stay deterministic/cacheable) — the agent sets it only when the question states one. Scalars are no longer pre-rounded (`answer(decimals=…)` controls precision). Reproduction is pinned by **scikit-learn reference-parity tests** (each op == a direct sklearn computation for the same setup). **`CACHE_VERSION` bumped 3 → 4** (ML op semantics + param names changed).
- **[Experimental / internal]** Métier analyst — **DA-bench quick wins** (visualization + shipped ML + insight). **Visualization breadth:** new chart ops `scatter`, `histogram`, `box`, `pie`, `heatmap` (of a correlation/pivot matrix) alongside `bar_chart`/`line_chart`, which gains a `series` for multi-line — all on a shared `_save_fig` helper. **`feature_importance(table, target, features, model=…)`** ranks features (impurity importances for rf/tree, `|coef|` for linear/logreg; one-hot columns summed back to the source feature; regression/classification task auto-inferred). **ML now ships in the frozen binary** — the PyInstaller build passes `--extra ml` so the specs bundle scikit-learn (previously `ml_*` ops errored in releases), with a smoke check asserting it's present. **`narrate` grounding:** its prompt now requires quoting the exact figures/columns and inventing nothing. No `CACHE_VERSION` bump (new ops + a non-deterministic prompt tweak).
- **[Experimental / internal]** Métier analyst — **LLM operators (insight, not just tables)**: `narrate(table, goal="…")` writes a short factual prose takeaway about a summary table, and `classify(table, column, labels=[…])` labels each row's text with one batched model call (adds a `{column}_label` column). Operators opt in with `@operator(needs_llm=True)` + a reserved `llm` parameter the **executor injects at run time** (it's not a graph param, so it stays out of the catalog, validation, and the recipe fingerprint); the caller is a deterministic (temperature-0) wrapper over the session backend (`MCPSamplingHandler.complete_text`). Results cache per recipe `(goal/labels + input fingerprint)` — a re-run is a cache hit, not a re-bill (change the goal or `/graph cache clear` to regenerate); LLM ops are exempt from `verify_purity`. Guards: input rows/tokens capped (`classify` errors rather than silently truncating past `max_rows`, and rejects a mismatched-count or out-of-set label set); headless/no-backend runs get a clear "needs a live session" error. No `CACHE_VERSION` bump (existing recipes unchanged).
- **[Experimental / internal]** Métier analyst — **per-step schema in every run result**, so the agent stops guessing columns and burning tool iterations. Each `run_pipeline`/`graph_patch` result now reports the **columns + inferred dtypes + row count** of each new/changed step (`node: col:dtype, … (N rows)`), so the agent sees the loaded data's real column names *and* when an op reshaped the table (e.g. `describe`) — the two things a live-session analysis (45% of tool calls failing) showed it was guessing at. Cheap (names + a bounded dtype sample; payloads stay in cache), shown only for added/changed nodes, wide tables capped; the dtype helper is shared with `graph_inspect` (`render.table_schema`/`render.infer_dtype`). The prompt now leads with the **typed stat ops** (`distribution`/`describe`/`quantile`/`outliers_iqr`) over hand-written SQL, and the DSL's mid-pipeline reference error explains how to fan out (`x = …` then `x | …`) or wire a kwarg (`t2=x`).
- **[Experimental / internal]** Métier analyst — **operator argument constraints surfaced + enforced**, so the agent stops guessing invalid values. Enum params are now typed with `Literal[...]` (a single source of truth): the always-on catalog shows each param's allowed values inline (e.g. `model:logreg|tree|rf`) **and** each operator's one-line description, alongside type + default. `validate` now checks param **values** before execution — an invalid enum fails with the allowed list and a "did you mean …?" hint, and a wrong type fails with `expected <type>, got <type>`. A coercion pass first fixes unambiguous slips (numeric string↔number, `"true"/"false"`→bool, number→str, a scalar where a `list` is expected → one-element list) so trivial mistakes just work; coercion runs before fingerprinting so the shared cache stays correct. Also: `read_csv`'s content-fingerprint autofill now expands `~` (home-relative paths like `~/data.csv` load instead of erroring).
- **[Experimental / internal]** Métier analyst — `derive_column` now supports **comparison operators** (`>`, `>=`, `<`, `<=`, `==`, `!=`) yielding a `1/0` column, so a binary target (e.g. `revenue > 1000`) no longer needs a `sql` CASE. Arithmetic (`+ - * /`) is unchanged.
- **[Experimental / internal]** Métier analyst — **statistics + machine-learning operators** and a single-value answer surface. New typed stats ops so the agent needn't hand-write DuckDB SQL: `describe` now also reports **p25/median/p75**, plus `quantile`, `outliers_iqr` (1.5·IQR fences + count), and `distribution` (skewness/kurtosis). New **optional `[ml]` extra** (scikit-learn, lazy-imported, install with `uv sync --extra ml`) adds `ml_regression` / `ml_classification` / `ml_cluster` — each fits a model on a train/test split and returns the held-out metric (r2/rmse/mae, accuracy/f1, silhouette), with categorical features one-hot encoded and a fixed `seed` for deterministic, cache-stable results. New `answer(decimals=…)` sink surfaces a single 1×1-table value as the exact reply (rounded), for "what is X?" questions. The analyst prompt now guides answering a specific question with the smallest pipeline ending in `answer`. **Cache-version bumped** (`describe`'s output shape changed). ML results are keyed by recipe only, so bump `CACHE_VERSION` when upgrading scikit-learn.
- **[Experimental / internal]** Métier DSL parser hardened against real agent output: pipelines fold across lines (leading- or trailing-`|`), triple-quoted strings (`"""…"""` / `'''…'''`) carry SQL verbatim (quotes/commas/newlines), statement-splitting is quote- and bracket-aware (a multi-line `sql(query="""…""")` **and** a single `op(...)` call whose arguments span several lines both stay one statement), trailing commas in an argument list are tolerated, an assignment (`name = …`) always starts its own statement, and a dangling trailing `|` is tolerated while `||`/stray pipes error clearly. Backed by a grown fuzz corpus of realistic programs.
- **[Experimental / internal]** Métier **shared, cross-session result cache**. The content-addressed result store moved from per-session (`<session_dir>/graph/cache.sqlite`) to a single shared store under `~/.vibe` (`graph-cache-v{N}.sqlite`), so an operation computed in one session is an instant cache hit in the next — re-opening a saved block or re-submitting the same pipeline recomputes nothing its recipe already covers (the executor already decides cached-vs-fresh purely by fingerprint lookup, so no executor change). Safe-guards: the filename carries a `CACHE_VERSION` (bump on any operator-semantics change → clean total invalidation); `PRAGMA busy_timeout` handles concurrent sessions; a soft size cap (512 MB) evicts oldest-first once per run (not per write). New `/graph cache` shows the shared store's size and `/graph cache clear --yes` purges it (recovery path for a poisoned entry or payload residue). Used by both the `graph` and `analyst` authoring agents.
- **[Experimental / internal]** Métier analyst — SQL-native compute + a pipeline-DSL authoring surface. The analyst now authors with **`run_pipeline`**: a compact program (`read_csv(...) | sql(...) | to_markdown()`) parsed into a cached graph and executed incrementally — resubmit the whole program to edit and only the changed steps recompute (replaces typed `graph_patch` node authoring for the analyst; the generic `graph` agent keeps `graph_patch`). A **`sql` operator backed by in-process DuckDB** is the workhorse: one node does filter/join/group/pivot/window over its wired tables (`t1`/`t2`/`t3`), sandboxed (`enable_external_access=False`, no file/network), lazily imported, a pure cached graph node. **Computed block params** let block authors bake templated inner params (e.g. `top_n.by="{metric}_sum"`) resolved at expand, so `rank_by`/`trend_by_period` no longer leak derived column names (+ new `month_over_month_growth` block). Shared execution/handle/catalog machinery is factored out of `graph_patch` (`build_result`/`format_llm_content`) and reused by `run_pipeline`. Adds a `duckdb` dependency (lazy-imported).

- **[Experimental / internal]** Métier analyst operators are now **pandas-backed**. Aggregation/stats/join and a batch of new reshape/window operators run on pandas (numpy under it), imported *lazily* so CLI startup stays pandas-free. The `{columns, rows}` wire format is unchanged (a JSON round-trip keeps values JSON-native — NaN→null, ISO dates — so the cache, `graph_inspect`, and `verify_purity` all keep working). New operators: `pivot`, `melt`, `concat`, `fill_missing`, `rank`, `bin`, `correlation`, `pct_change`, `rolling`. New data-quality gates: `expect_columns`, `expect_no_nulls`, `expect_unique` (fail fast with an actionable error). New sinks that write a file and return a small handle (kept out of context): `to_csv`, and matplotlib (`Agg`) `bar_chart`/`line_chart`. `join` is now a real merge (dup keys multiply, standard SQL semantics) rather than a lookup. Adds `pandas`/`numpy`/`matplotlib` as dependencies.
...
- **[Experimental / internal]** Métier management/visualization commands: `/graph` opens a full-screen interactive panel — a navigable node list coloured by run state beside a live per-node details pane (op, inputs, params, cache status, output), with `c` to copy a Mermaid diagram and `esc` to close — and `/blocks` lists the saved-block library (`/blocks show <name>` opens the same panel on a block's subgraph; `/blocks rm <name> --yes`). Backed by UI-free renderers in `vibe/core/graph/render.py`; `graph_patch` now also persists each run's report to `<session_dir>/graph/report.json`.
- **[Experimental / internal]** Métier catalog & review polish: operators now carry arg **types** + a description; a new **`/operators`** command lists the typed operator/block catalog for humans; the `graph_patch` **approval gate renders a readable per-op diff** (add/set/connect/…) instead of raw JSON; and each patch's tool-result line **glimpses** the fresh/cached counts + a snippet of the terminal output.
- **[Experimental / internal]** Métier context-as-handles: a new `BaseTool.get_llm_content` hook lets a tool replace its model-facing text (the agent loop's default is still the full field dump, so every other tool is unchanged). `graph_patch` uses it to send the model **content-addressed handles** (`ref:<fingerprint> <Type> — <preview>`) for terminal outputs instead of the full payloads — intermediate tables stay in the cache, out of context — and to send the operator/block **catalog only once per session** (an empty patch re-lists it). The full result still reaches the UI and cache untouched.
- **[Experimental / internal]** Métier use-case comparison: deterministic **analytics** (`load_events → filter → join → group_by → top_n → report`) and research-shaped operator kits (`vibe/core/graph/demo/analytics.py`, `demo/research.py`) with large intermediates, plus a head-to-head harness `python -m vibe.core.graph.demo.compare` that measures the graph vs. a plain transcript agent on **context size** (≈99% fewer tokens: payloads live in the cache, not the transcript) and **recompute cost** (a re-run recomputes only the dirty subgraph). Both kits are registered as part of the `graph` agent's operator library, so the live agent can build these workflows directly.
- **[Experimental / internal]** Métier **`analyst` agent** — a data-analysis-focused agent (`vibe --agent analyst`). Adds per-agent **catalog scoping** (operators/blocks carry a `library` tag; `graph_patch` shows and accepts only the agent's library, the generic `graph` agent still sees everything), a curated **data-analysis operator library** (`vibe/core/graph/library/analysis.py`: one generic `Table` through `read_csv`/`sample_dataset` → `select`/`filter`/`cast`/`drop_missing`/`derive`/`date_part`/`join` → `group_by`/`describe`/`value_counts`/`top_n` → `to_markdown`), classic-analysis **blocks** (`quick_profile`, `rank_by`, `trend_by_period`, `segment_summary`), and a read-only **`graph_inspect`** tool (a node's schema + inferred dtypes + sample rows). `read_csv` loads a real file the agent names — `graph_patch` auto-computes the content-hash fingerprint each turn, so editing the file re-runs only the dependent subgraph. The synthetic `demo/analytics.py` kit was retired in favor of this library (the comparison harness now uses it).
