...
- **[Experimental / internal]** Métier **`doc-reviewer` agent** — a second métier (`vibe --agent doc-reviewer`) that proves the workflow-graph engine is **artifact-agnostic**: instead of the analyst's tabular `Table`, a `Document` (a contract split into `Section`s) flows through the *same* engine — handles, content-addressed cache, incremental recompute, and the `run_pipeline` DSL all work unchanged, with no engine edits (the handle keys off `type(result).__name__` and the excerpt off `model_dump_json()`, so a `Document` renders as `ref:<fp> Document` for free). New scoped operator library (`vibe/core/graph/library/doc_reviewer.py`, tagged `library="documents"`): load/segment (`read_document` — `.txt`/`.md` native, `.pdf` via the lazy optional **`[pdf]`** extra; `sample_contract`), shape/guard (`filter_sections`, `expect_sections`), extract/analyze (LLM-backed `extract_clauses`, `classify_risk`, `compare_to_playbook`, `summarize_document`, `redline`; the pure `find_missing_clauses`), and report sinks (`outline`, `findings_to_markdown`). LLM ops reuse the analyst's `@operator(needs_llm=True)` + executor-injected `llm` machinery (strict validatable JSON contracts, input caps that error rather than truncate, cache per recipe). Classic-review **blocks**: `risk_review`, `clause_inventory`, `missing_clauses`. Scoped catalog (the reviewer sees only the `documents` library; the analyst/graph agents are unchanged), a dedicated `doc_reviewer` system prompt, bundled sample contracts + a playbook, and a **comparison-harness** row (`python -m vibe.core.graph.demo.compare`) showing the same context / recompute wins on a `Document` workflow (~85% cold / ~99% re-run context reduction; only the dirty subgraph recomputes). New optional `[pdf]` extra (`pypdf`, lazy-imported; `.txt`/`.md` need no extra). No `CACHE_VERSION` bump (a new, independent library).
