...
- **[Experimental / internal]** Métier management/visualization commands: `/graph` renders the current session's workflow graph (tree + per-node last-run state and cache status + a copyable Mermaid diagram; `/graph <node>` inspects one node), and `/blocks` lists the saved-block library (`/blocks show <name>`, `/blocks rm <name> --yes`). Backed by UI-free renderers in `vibe/core/graph/render.py`; `graph_patch` now also persists each run's report to `<session_dir>/graph/report.json`.
